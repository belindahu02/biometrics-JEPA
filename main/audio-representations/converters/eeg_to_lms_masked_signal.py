...
Example:
    ...
    python eeg_to_lms.py /your/eeg/data /your/eeg_lms_output --enable_masking --min_mask_length 0.1 --max_mask_length 2.0
"""
import numpy as np
...
def apply_random_masking(signal, sample_rate, min_mask_duration=0.25, max_mask_duration=1, num_masks=2):
    """
    Apply random masking to a signal by setting random blocks to zero.

    Args:
        signal: Input signal array
        sample_rate: Sampling rate of the signal
        min_mask_duration: Minimum duration of mask blocks in seconds
        max_mask_duration: Maximum duration of mask blocks in seconds
        num_masks: Number of mask blocks to apply
    ...
    # # Ensure mask lengths don't exceed signal length
    # min_mask_samples = min(min_mask_samples, signal_length // 4)  # Max 25% of signal
    # max_mask_samples = min(max_mask_samples, signal_length // 2)  # Max 50% of signal

    if min_mask_samples <= 0 or max_mask_samples <= 0:
        return signal_masked  # Skip masking if signal is too short

    for i in range(num_masks):
        # Randomly choose mask length from uniform distribution
        mask_length = np.random.randint(min_mask_samples, max_mask_samples + 1)

        # Randomly choose start position ensuring mask fits within signal
        max_start = max(0, signal_length - mask_length)
        if max_start <= 0:
            continue  # Skip this mask if it doesn't fit

        start_pos = np.random.randint(0, max_start + 1)
        end_pos = start_pos + mask_length

        # Apply mask (set to zero)
        signal_masked[start_pos:end_pos] = 0.0
...
def create_overlapping_frames(spectrogram, frame_duration, frame_stride, hop_size, sample_rate):
    ...
    (Kept for compatibility - not used in masking workflow)
    ...
    try:
        ...
        for channel_idx in range(n_channels):
            try:
                ...
                # Apply random masking if enabled
                if masking_params['enable_masking']:
                    eeg_signal = apply_random_masking(
                        eeg_signal,
                        prms.sample_rate,
                    ...
                    if verbose:
                        print(f'  Applied random masking to channel {channel_idx} ({channel_name})')
        ...
        if verbose:
            ...
            print(f'Saved {total_frames_saved} total frames from {len(successful_channels)}/{n_channels} channels for {edf_input_path.name} {masking_status}')
...
def convert_eeg_batch(base_from_dir, base_to_dir,
    ...
    Args:
        ...
        enable_masking: Whether to apply random masking to signals
        min_mask_length: Minimum duration of mask blocks in seconds (default: 0.5)
        max_mask_length: Maximum duration of mask blocks in seconds (default: 2.0)
        num_masks: Number of mask blocks to apply per signal (default: 2)
    """
    ...
    if enable_masking:
        print(f'Random masking ENABLED:')
        print(f'  - Number of masks per signal: {num_masks}')
        print(f'  - Mask duration range: {min_mask_length}s - {max_mask_length}s')
        print(f'  - Masks are applied before spectrogram conversion')
    else:
        print(f'Random masking DISABLED')
    ...
    for i in range(start_subject_id, end_subject_id + 1):
        ...
        for f in subject_edf_files:
            worker_args_for_subject.append([f, str(current_subject_from_dir), str(current_subject_to_dir), prms, to_lms, suffix, min_length, verbose, masking_params])
        ...
        for subpathname_relative_to_subject_dir, from_dir_worker, to_dir_worker, _, _, _, _, _, _ in worker_args_for_subject:
...
if __name__ == "__main__":
    ...
    convert_eeg_batch("/app/1.0.0","/app/data/masked_subset10", 1, 10)
