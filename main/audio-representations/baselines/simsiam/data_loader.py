...
def norma_pre(x_all):
    """
    Normalize input data across the last dimension
    using mean=0 and std=1 scaling (like sklearn's StandardScaler).
    """
    # Flatten temporal + batch dimensions
    x = np.reshape(x_all, (x_all.shape[0] * x_all.shape[1], x_all.shape[2]))
...
def aug_data(x_train, y_train, transformations, sigma_l, ext):
    ...
    Applies transformations to x_train and creates labels in y_train_pro.
    """
    ...
    for j in range(num_sample):
        ...
        for i, (Jt, sigma) in enumerate(zip(transformations, sigma_l)):
            # Original
            ...
            if ext:
                ...
                for k, trans in enumerate(transformations):
                    if i != k:
                        x_train_pro[i, j * m_ + 1 + cnt, :, :] = np.copy(trans(x_train_temp, sigma=sigma_l[k]))
