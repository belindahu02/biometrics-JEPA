...
class M2DModule(LightningModule):
    ...
    def training_step(
        ...
        """
        inputs = batch[0]
        context_mask, target_mask = self.encoder.generate_masks(inputs)
        latents = self.encoder(inputs, mask=context_mask)
        preds = self.predictor(latents, context_mask=context_mask, target_mask=target_mask)

        with torch.no_grad():
            targets = self.target_encoder(inputs, mask=target_mask)

        # TODO: think of how we compute loss based on mask
...
if __name__ == "__main__":
    _ = M2DModule(None, None, None, None, )
