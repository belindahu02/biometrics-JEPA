...
def eeg_to_spectrogram(
    ...
    save_format: str = "npy",  # New parameter: "npy" or "png"
    ...
    Args:
        ...
        figsize (tuple): Figure size for the spectrogram (default: (3.84, 3.84))
    ...
    try:
        ...
        if save_format == "npy":
            # Save as NumPy array
            # You can choose to save just the power spectral density (Sxx) or all three arrays
            # Option 1: Save only the spectrogram data (Sxx)
            np.save(output_path, Sxx)
            
            # Option 2: Save all three arrays as a dictionary (uncomment if preferred)
            # spectrogram_data = {
            #     'frequencies': frequencies,
            #     'times': times,
            #     'spectrogram': Sxx
            # }
            # np.save(output_path, spectrogram_data)
...
def batch_eeg_to_spectrogram(
    ...
    save_format: str = "npy",  # New parameter
    **kwargs
) -> list:
    """
...
if __name__ == "__main__":
    # Single file processing to NPY
    try:
        dataset_path = "../mmi/dataset/physionet.org/files/eegmmidb/1.0.0"
        
        # Process to .npy files - will save to spectrograms/npy/S001/, spectrograms/npy/S002/, etc.
        process_subject_folders(
            dataset_path,
            subject_pattern="S00?",  # S001-S009
            save_format="npy",  # Save as NumPy arrays
            output_directory="datasets"  # Files will be saved to spectrograms/npy/
        )
        
        # Process to .png files - will save to spectrograms/png/S001/, spectrograms/png/S002/, etc.
        # process_subject_folders(
        #     dataset_path,
        #     subject_pattern="S00?",
        #     save_format="png",  # Save as PNG images
        #     output_directory="spectrograms"  # Files will be saved to spectrograms/png/
        # )
        
        # Example of loading a saved spectrogram
        # spectrogram_data = load_spectrogram_npy("spectrograms/npy/S001/S001R01_spectrogram.npy")
        # print(f"Loaded spectrogram shape: {spectrogram_data.shape}")
